...
public class UgandaEMRPOCServiceImpl extends BaseOpenmrsService implements UgandaEMRPOCService {
    ...
    public SimpleObject getProcessedOrders(String query, Date asOfDate, boolean includeProccesed) throws ParseException, IOException {
        ...
        query = String.format(query, QueueingUtil.dateFormtterString(date, DAY_START_TIME), QueueingUtil.dateFormtterString(date, DAY_END_TIME));
     ...
     * @param completePreviousQueueStatus
     * @param nextQueueStatus
     * @param completePreviousQueue
     */
    public void sendPatientToNextLocation(FormEntrySession session, String locationToUUID, String locationFromUUID, String completePreviousQueueStatus, String nextQueueStatus, boolean completePreviousQueue) {
        ...
        try {
            if (!patientQueueExists(session.getEncounter(), locationTo, locationFrom,nextQueueStatus)) {
                if (completePreviousQueue) {
                    completePreviousQueue(session.getPatient(), session.getEncounter().getLocation(), completePreviousQueueStatus, QUEUE_STATUS_PENDING);
                }
    ...
    public Encounter processLabTestOrdersFromEncounterObs(FormEntrySession session, boolean completePreviousQueue) {
        ...
        if (!orders.isEmpty()) {
            ...
            if (!session.getEncounter().getOrders().isEmpty()) {
                sendPatientToNextLocation(session, LAB_LOCATION_UUID,encounter.getLocation().getUuid(), QUEUE_STATUS_SENT_TO_LAB, QUEUE_STATUS_PENDING, completePreviousQueue);
            }
        }
        return encounter;
    }
